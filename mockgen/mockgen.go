...
const (
	gomockImportPath = "github.com/bitgaming/mock/gomock"
)
...
func (g *generator) Generate(pkg *model.Package, outputPkgName string, outputPackagePath string) error {
	...
	g.p("// Code generated by MockGen-RSpec. DO NOT EDIT.")
	if g.filename != "" {
...
func (g *generator) GenerateMockInterface(intf *model.Interface, outputPackagePath string) error {
	...
	g.p("")
	for _, m := range intf.Methods {
		g.p("calls%v []call%v%v", m.Name, intf.Name, m.Name)
	}
	...
	// "allow" type for each interface method
	for _, m := range intf.Methods {
		g.p("type allow%v%v struct {", intf.Name, m.Name)
		g.in()
		for i, out := range m.Out {
			g.p("ret%v %v", i+1, out.Type.String(g.packageMap, outputPackagePath))
		}
		g.out()
		g.p("}")
	}
	...
	// "call" type for each interface method
	for _, m := range intf.Methods {
	...
	// "receiver" type
	g.p("type receiver%v struct {", intf.Name)
	g.in()
	g.p("allower *allower%v", intf.Name)
	g.out()
	g.p("}")
	g.p("")

	// "receiver" type for each interface method
	for _, m := range intf.Methods {
	...
	g.p("type ErrorReporter%v interface {", intf.Name)
	g.in()
	g.p("Errorf(format string, args ...interface{})")
	g.out()
	g.p("}")
	...
	for _, m := range intf.Methods {
		...
		for i, _ := range m.Out {
			rets[i] = fmt.Sprintf("m.allow%v.ret%v", m.Name, i+1)
		}
		g.p("return %v", strings.Join(rets, ", "))
	...
	g.in()
	g.p("m.t.Errorf(\"`%%v` does not match expectation:\\nWant: %%v\\nGot : %%v\", argName, string(formattedWant), string(formattedGot))")
	g.out()
	g.p("} else {")
	g.in()
	g.p("m.t.Errorf(\"`%%v` does not match expectation:\\nWant: %%#v\\nGot : %%#v\", argName, want, got)")
	g.out()
	g.p("}")
	g.p("m.printStack()")
	g.out()
	...
	g.in()
	g.p("if strings.Contains(arrayStack[i], \".go\") && !strings.Contains(arrayStack[i], \"/goblin\") {")
	g.in()
	g.p("m.t.Errorf(arrayStack[i])")
	g.out()
	g.p("}")
	g.out()
	g.p("}")
	...
	g.p("func (m *allower%v) ToReceive() *receiver%v {", intf.Name, intf.Name)
	g.in()
	g.p("return &receiver%v{allower: m}", intf.Name)
	g.out()
	g.p("}")
	g.p("")
	...
	for _, m := range intf.Methods {
		...
		rets := make([]string, len(m.Out))
		for i, p := range m.Out {
			rets[i] = fmt.Sprintf("ret%v %v", i+1, p.Type.String(g.packageMap, outputPackagePath))
		}
	...
	g.p("func (m *expecter%v) ToHaveReceived(expectedFuncName ...string) *expecterHelper%v {", intf.Name, intf.Name)
	g.in()
	g.p("if len(expectedFuncName) > 1 {")
	g.in()
	g.p("m.mock.errorf(\"Use one function name only, not %%v\", len(expectedFuncName))")
	g.out()
	g.p("}")
	g.p("if len(expectedFuncName) == 1 {")
	g.in()
	...
	g.p("func (m *expecterHelper%v) TIMES(n int) {", intf.Name)
	g.in()
	for _, m := range intf.Methods {
		...
		g.p("if callCount != n {")
		g.in()
		g.p("m.expecter.mock.errorf(\"Expected %v to have been called %%v times but it was called %%v time(s)\", n, callCount)", m.Name)
		g.out()
		g.p("}")
		g.p("return")
		g.out()
	...
	g.p("m.expecter.mock.errorf(\"%%v is not a method of %v\", m.expectedFuncName)", intf.Name)
	g.out()
	g.p("}")
